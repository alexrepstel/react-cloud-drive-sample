...
class RenameFolderPopup extends React.Component {
    constructor(props){
        super(props);
        this.state = { 
      };
    ...
    handleSubmit(event) {
      ...
      const options = {
        url: 'http://localhost:5000/driveAPI/renameFolder',
        method: 'POST',
        headers: {
  ...
  render() {
    ...
    <h3 className="add-new-item-title">Rename folder</h3>
        <form onSubmit={this.handleSubmit}>
        <p>New folder name:</p>
        <input className="add-new-folder-input" name="folder_label" type="text" placeholder={this.props.folderName}></input>
        <input name="current_folder_id" value={current_folder_id} hidden></input>
        <input name="this_file_id" value={this.props.item_id} hidden></input>
